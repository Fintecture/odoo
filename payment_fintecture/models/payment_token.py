...
class PaymentToken(models.Model):
    ...
    def _fintecture_sca_migrate_customer(self):
        """ Migrate a token from the old implementation of Fintecture to the SCA-compliant one.

        In the old implementation, it was possible to create a Charge by giving only the customer id
        and let Fintecture use the default source (= default payment method). Fintecture now requires to
        ...
        response_content = self.acquirer_id._fintecture_make_request(
            'payment_methods',
            payload={
        ...
        if not payment_method_id:
            raise ValidationError("Fintecture: " + _("Unable to convert payment token to new API."))
        self.fintecture_payment_method = payment_method_id
